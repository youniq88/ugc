...
- `?preset=planets` / `gems` / `jelly` / `myth` / `odyssey` / `dogs` / `evolution` / `starlife` / `money` / `fruits` : 프리셋 선택 (URL 파라미터)
- `?ui=0` : 버튼 UI 숨긴 채 시작
- `?finale=lightning` / `?finale=arrow` / `?finale=moon` / `?finale=confetti` : 마지막 단계에 도달했을 때의 피날레를 지정 (기본값은 프리셋마다 다름)
...
기본은 색종이 축하 연출입니다. 프리셋의 `finale` 값이나 `?finale=` 파라미터로 바꿀 수 있습니다: `lightning` 번개(신화 프리셋 기본), `arrow` 화살(오디세이아 프리셋 기본), `moon` 달빛(강아지 프리셋 기본), `meteor` 운석이 떨어져 전부 불타고 작은 한 마리만 날아오름, `ash` 화산재가 쏟아져 전부 회색으로 굳음, `implode` 낮은 단계부터 수압에 찌그러져 터지고 마지막 단계만 남음. 뒤의 세 가지는 아직 어느 프리셋에도 붙어 있지 않습니다.
...
- claude.ai 의 Routine "머지 게임 릴스 매일 자동 제작·업로드" 가 매일 08:00 KST(23:00 UTC) 에 새 세션을 열어, 프리셋을 하루씩 돌아가며 고르고(planets → jelly → myth → odyssey → dogs → evolution → starlife → money → gems → fruits), Higgsfield 샌드박스에서 이 저장소의 `index.html` + `tools/record.js` 로 녹화한 뒤 mp4 를 Higgsfield CDN 에 올리고, Zapier 로 YouTube(쇼츠, 공개)와 Instagram(릴스)에 캡션 하나와 함께 올립니다. 캡션과 제목은 영어 한 줄로, 해시태그 없이, 결말을 밝히지 않고 끝까지 보고 싶어지게 씁니다.
