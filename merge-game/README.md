...
기본은 색종이 축하 연출입니다. 프리셋의 `finale` 값이나 `?finale=` 파라미터로 바꿀 수 있습니다: `lightning` 번개(신화 프리셋 기본), `meteor` 운석이 떨어져 전부 불타고 작은 한 마리만 날아오름, `ash` 화산재가 쏟아져 전부 회색으로 굳음, `implode` 낮은 단계부터 수압에 찌그러져 터지고 마지막 단계만 남음. 뒤의 세 가지는 아직 어느 프리셋에도 붙어 있지 않습니다.
